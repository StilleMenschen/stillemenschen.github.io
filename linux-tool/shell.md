...
> 如果要在`[`到`]`之间指定匹配`-`字符，可以将`-`放在`[`后面或`]`前面，如`[a-zA-Z0-9-]`
...
在交互执行时， bash 在准备好读入一条命令时显示主提示符 `PS1`， 在需要更多的输入来完成一条命令时显示 PS2。
Bash 允许通过插入一些反斜杠转义的特殊字符来定制这些提示字符串，这些字符被如下解释：
...
1. 批量处理当前目录下的所有 Git 项目更新
   ...
   for repo in $(find -maxdepth 2 -type d -path '*/.git' | cut -d/ -f 2); do
       cur_dir=${parent_dir}/${repo}
       if [ -d "${cur_dir}" ]; then
           echo ${repo};
           cd ${cur_dir};
           git fetch --all;
