...
| -writable          | 匹配可写文件                                                                                                                      |
| -newer file        | 匹配数据最后修改时间比文件 `file` 更新的文件                                                                                      |
| -anewer file       | 匹配上次访问时间比文件 `file` 更新的文件                                                                                          |
| -cnewer file       | 匹配状态更改时间比文件 `file` 更新的文件                                                                                          |
| -newerXY reference | 高级比较：将当前文件的 `X` 属性与 `reference` 的 `Y` 属性进行比较。`Y` 可以是一个文件，也可以是一个时间字符串（当 Y 为 `t` 时）。 |

### 关于 `-newerXY` 的说明：

这是一个非常强大的选项，其中 **X** 和 **Y** 可以是以下字母：

- `a`: 文件的最后访问时间 (Access time)
- `c`: 文件状态最后改变时间 (Status change time)
- `m`: 文件的最后修改时间 (Modification time)
- `B`: 文件的创建时间 (Birth time，部分系统或文件系统不支持)
- `t`: 将 **reference** 视为一个绝对时间字符串（仅用于 **Y**）

常用组合示例：

- `-newermt "2023-01-01"`：查找修改时间晚于 2023 年元旦的文件。
- `-newermm file2`：等同于 `-newer file2`。
- `-neweram file2`：查找“最后访问时间”比“file2 的修改时间”更新的文件。
...
4. 查找`/var/logs`目录中修改时间在 8 天前大于 30M 的 log 文件并删除

   ```bash
   find /var/logs -name '*.log' -mtime +7 -size +30M -delete
   ```

5. 查找修改时间在前一天凌晨 0 点之前的文件

   ```bash
   find /path/to/search -type f -not -newermt "yesterday 00:00:00" -ls
   ```

6. 查找比某个特定文件更新（修改时间更晚）的所有文件（常用于增量备份场景）

   ```bash
   find . -type f -newer reference.txt
   ```

7. 查找在特定日期范围内修改的文件（2023-10-01 到 2023-10-31 之间）

   ```bash
   find /path/to/search -type f -newermt "2023-10-01" ! -newermt "2023-11-01"
   ```

8. 查找 1 小时内被修改过的文件

   ```bash
   find . -type f -newermt "1 hours ago"
   ```
