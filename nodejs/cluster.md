...
// nodejs v16.0.0 之前使用 cluster.isMaster
if (cluster.isPrimary) {
  /*主进程*/
  let totalTask = 0;
  console.log(`[Main ${process.pid}] started`);
  for (let taskNum = 0; taskNum < numberOfCores; taskNum++) {
    const worker = cluster.fork();
    worker.send(totalTask++);
  }
  cluster.on("message", (worker, message) => {
    console.log(`[Main ${process.pid}] Worker ${worker.id}: ${message}`);
    worker.disconnect();
  });
  cluster.on("exit", (worker, code, signal) => {
    console.log(`[Main ${process.pid}] Worker ${worker.id} died. code:${code} signal:${signal}`);
  });
} else {
  /*子进程*/
  process.on("message", (id) => {
    console.log(`[Worker ${process.pid}] Start computing ${id}`);
    let sourceList = [];
    for (let i = 0; i < 10; i++) {
      sourceList.push(getRandomNumber(1, 100));
    }
    console.log(`[Worker ${process.pid}] Before ${sourceList.join(", ")}`);
    sourceList = quicksort(sourceList);
    console.log(`[Worker ${process.pid}] After ${sourceList.join(", ")}`);
    process.send(`Task ${process.pid} has ended.`);
  });
}
```
