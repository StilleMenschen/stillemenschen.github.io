...
const Coffee = {
  props: ["name", "age"],
  template: "<div>coffee name: {{ name }}, age: {{ age }}</div>",
};
...
const routerTable = {
  "/foo/:id": Foo,
  "/bar": Bar,
  "/coffee/:name/:age": Coffee,
};

const compiledRoutes = [];
Object.keys(routerTable).forEach((path) => {
  const dynamicSegments = [];
  const regexp = pathToRegexp(path, dynamicSegments);
...
const app = new Vue({
  ...
  render(h) {
    const path = "/".concat(this.url);

    let componentToRender = NotFound;
    let props = {};

    compiledRoutes.some(({ component, regexp, dynamicSegments }) => {
    ...
    return h("div", [
      h(componentToRender, { class: "view", props }),
      h("a", { attrs: { href: `#foo/123` } }, "foo 123"),
      h("a", { attrs: { href: `#foo/234` } }, "foo 234"),
      h("a", { attrs: { href: `#bar` } }, "bar"),
      h("a", { attrs: { href: `#coffee/Jack/18` } }, "coffee Jack 18"),
    ]);
  },
});
```
...
详细可参考框架的[GitHub 主页](https://github.com/pillarjs/path-to-regexp)
